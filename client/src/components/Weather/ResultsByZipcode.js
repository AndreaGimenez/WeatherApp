...
class ResultsByZipcode extends Component {
  constructor(props){
    super(props);
    this.state = {
      data : null,
  ...
  callBackendAPI = async () => {
    const response = await fetch('/search-location-weather/zipcode/' + this.props.match.params.zipcode);
  ...
  render() {
    return (
      <div className="ResultsByZipcode">
        { this.state.loaded ?
            <WeatherInfo
...
export default ResultsByZipcode;
